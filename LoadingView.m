...
+ (id)loadingViewInView:(UIView *)aSuperview
{
	...
	UILabel *loadingLabel =
		[[[UILabel alloc]
			initWithFrame:labelFrame]
		autorelease];
	loadingLabel.text = NSLocalizedString(@"Connecting...", nil);
	loadingLabel.textColor = [UIColor whiteColor];
	loadingLabel.backgroundColor = [UIColor clearColor];
	loadingLabel.textAlignment = UITextAlignmentCenter;
	loadingLabel.font = [UIFont boldSystemFontOfSize:[UIFont labelFontSize]];
...
- (void)stopActivity{
    for(UIView *aSubview in self.subviews){
        if([aSubview isKindOfClass:[UIActivityIndicatorView class]]){
            [(UIActivityIndicatorView*)aSubview stopAnimating];
        }
    }
}
...
- (void)startActivity{
    for(UIView *aSubview in self.subviews){
        if([aSubview isKindOfClass:[UIActivityIndicatorView class]]){
            [(UIActivityIndicatorView*)aSubview startAnimating];
        }
    }
}
...
- (void)updateTextAndRemoveView:(NSString *)text{
    ...
    [self performSelector:@selector(stopActivity) withObject:nil afterDelay:0.7];
...
- (void)removeView
{
	...
	if([delegate respondsToSelector:@selector(loadingViewDidClose:)])
		[delegate loadingViewDidClose:self];
}
